import React from "react";
import "../styles/Profile.css";
import "../styles/sign-up.css";
import "../styles/journal.css";
import axios from "axios";
import { useState, useEffect } from "react";
import Header from "../components/Header";
import userp from "../assets/user.png";

function Profile() {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [location, setLocation] = useState("");
  const [date, setDate] = useState("");
  const [photo, setPhoto] = useState(null);
  const [trips, setTrips] = useState([]);

  useEffect(
    () => {
      axios
        .get(
          "https://wanderlog-ab438-default-rtdb.europe-west1.firebasedatabase.app/Trips.json"
        )
        .then((res) => setTrips(res.data))

        .catch((err) => err);
    },

    // For calling the function one time
    []
  );

  function addJournalHandle(e) {
    e.preventDefault();
    axios
      .put(
        `https://wanderlog-ab438-default-rtdb.europe-west1.firebasedatabase.app/Trips/${trips.length}.json`,
        {
          Title: title,
          Description: description,
          Location: location,
          Date: date,
          Photo: photo,
          id: trips.length,
          isActive: false,
        }
      )
      .catch((err) => err);

    alert("Your Trip Has Been Added! ");
    [];
  }

  return (
    <>
      <Header />

      <div className="wrapper">
        <div className="profile-c">
          <h2 className="title mt-6">Profile</h2>

          <form className="profile" action="">
            <img className="userp" src={userp} alt="" />
            <div className="profile-group">
              <label className="profile-l" htmlFor="">
                First Name
              </label>
              <input
                placeholder="Please Write The First Name"
                className="user-name"
                type="text"
                name=""
                id=""
              />
            </div>
            <div className="profile-group">
              <label className="profile-l" htmlFor="">
                Last Name
              </label>
              <input
                placeholder="Please Write The Last Name"
                className="user-name"
                type="text"
                name=""
                id=""
              />
            </div>
            <div className="profile-group">
              <label className="profile-l" htmlFor="">
                First Name
              </label>
              <input
                placeholder="Please Write The Title"
                type="file"
                name=""
                id=""
              />
            </div>

            <div className="edit-btns flex justify-center mt-8">
              <button className="edit-btn">Edit</button>
            </div>
          </form>
        </div>

        <div className="journal-c">
          <h2 className="title mt-6">Journal</h2>
          <form className="entry-form" action="">
            <label htmlFor="">Title:</label>
            <input
              placeholder="Please Write The Title"
              onChange={(e) => setTitle(e.target.value)}
              className="j-input"
              type="text"
              name=""
              id=""
            />
            <label htmlFor="">Description:</label>
            <input
              placeholder="Please Write The Description"
              onChange={(e) => setDescription(e.target.value)}
              className="j-input"
              type="text"
              name=""
              id=""
            />
            <label htmlFor="">Location:</label>
            <input
              placeholder="Please Write The Location"
              onChange={(e) => setLocation(e.target.value)}
              className="j-input"
              type="text"
              name=""
              id=""
            />
            <label htmlFor="">Date:</label>
            <input
              placeholder="Please Write The Date"
              onChange={(e) => setDate(e.target.value)}
              className="j-input"
              type="date"
              name=""
              id=""
            />
            <label htmlFor="">Photo:</label>
            <input
              placeholder="Please Upload The Photo"
              onChange={(e) => setPhoto(e.target.value)}
              className="j-input"
              type="file"
              name="file"
              id=""
            />

            <button className="j-btn" onClick={addJournalHandle}>
              Add Journal
            </button>
          </form>
        </div>
      </div>
    </>
  );
}

export default Profile;
